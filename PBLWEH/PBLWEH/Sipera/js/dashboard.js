document.addEventListener("DOMContentLoaded", () => {
  ...
  const allRooms = [
    ...
    {id: 4,name: "Ruang GU 706",image: "foto/room.jpg",status: "Tersedia",},
  ...
  let allMeetings = [
    {
      id: 101,
      kode: "PBL777",
      judul: "Rapat Finalisasi Laporan PBL",
      tanggal: "2025-10-20",
      waktu: "10:00",
      ruanganId: 3,
      ruanganName: "Laboratorium Jaringan",
      ketuaId: "user-chandra",
      anggotaIds: [],
    },
  ];
  ...
  const kodeRapatBtn = document.querySelector(
    ".sidebar-feature .input-group button"
  );
  const navListRuangan = document.getElementById("nav-list-ruangan");
  const searchInput = document.getElementById("search-input"); // Tambahkan referensi ke search input
  ...
  const renderRapatSayaView = () => {
    ...
    const myMeetings = allMeetings.filter(
      (m) =>
        m.ketuaId === session.userId || m.anggotaIds.includes(session.userId)
    );
    const tbody = document.getElementById("rapat-saya-tbody");

    if (myMeetings.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align:center;">Anda belum terlibat dalam rapat apapun.</td></tr>';
      return;
    }

    tbody.innerHTML = myMeetings
      .map((meeting, index) => {
  ...
  const renderListRuanganView = (filter = "") => {
    ...
    if (filter) {
      ...
      sortedRooms = sortedRooms.filter(
        (room) =>
          ...
          room.status.toLowerCase().includes(filter)
      );
    }
    ...
    roomGrid.innerHTML = sortedRooms
      .map(
        (room) => `
            <div class="room-card">
                <div class="room-card-image"><img src="${
                  room.image || "../foto/default.jpg"
                }" alt="${room.name}"></div>
                <div class="room-card-content">
                    <h3>${room.name}</h3>
                    <p>Status: <strong>${room.status}</strong></p>
                    <div class="room-card-actions">
                        <button class="btn-secondary btn-reservasi" data-id="${
                          room.id
                        }" ${
          room.status === "Terpakai" ? "disabled" : ""
        }>Reservasi</button>
                    </div>
                </div>
            </div>
  ...
  const renderReservasiView = (room) => {
    ...
    document.getElementById("reservasi-form").dataset.roomId = room.id;
  };
  ...
  const renderRapatDetailView = (meeting, mode = "lihat_detail") => {
    ...
    if (mode === "sukses_reservasi") {
      ...
      subtitleEl.textContent =
        "Anda sekarang adalah Ketua untuk rapat berikut:";
    } else {
      titleEl.textContent = "Detail Rapat";
      subtitleEl.textContent =
        "Informasi lengkap mengenai rapat yang Anda ikuti.";
    }
  ...
  const handleJoinMeetingByCode = () => {
    ...
    if (!inputKode) {
      alert("Silakan masukkan Kode Rapat.");
      return;
    }
    ...
    if (foundMeeting) {
      const userAlreadyJoined =
        foundMeeting.anggotaIds.includes(session.userId) ||
        foundMeeting.ketuaId === session.userId;
      if (userAlreadyJoined) {
        alert("Anda sudah tergabung dalam rapat ini.");
        return;
      }
  ...
  const updateDynamicMenus = () => {
    ...
    const isKetuaInAnyMeeting = allMeetings.some(
      (m) => m.ketuaId === session.userId
    );
  ...
  const initializeDashboard = () => {
    renderListRuanganView(); // Panggil dengan filter kosong awal
    mainContent.addEventListener("click", handleMainContentClick);
    kodeRapatBtn.addEventListener("click", handleJoinMeetingByCode);
    rapatSayaLink.addEventListener("click", (e) => {
      e.preventDefault();
      renderRapatSayaView();
    });
    navListRuangan.addEventListener("click", (e) => {
      e.preventDefault();
      renderListRuanganView();
    });
    document
      .getElementById("kembali-btn")
      .addEventListener("click", renderListRuanganView);
    document
      .getElementById("reservasi-form")
      .addEventListener("submit", (e) => {
        e.preventDefault();
        handleReservasiSubmit();
      });
    document
      .getElementById("kembali-ke-list")
      .addEventListener("click", renderRapatSayaView);
    updateDynamicMenus();
    ...
    if (searchInput) {
      searchInput.addEventListener("input", () => {
        ...
        renderListRuanganView(filter); // Panggil render dengan filter dari input
      });
    }
  };
  initializeDashboard();
